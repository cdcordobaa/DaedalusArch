...
**Input to the LLM Critic** (per evaluation):

- Code snippet (relevant file or class)
- APG subgraph context (the node and its immediate neighborhood)
- Architectural rule in natural language (from ADR or spec)
- Evaluation rubric (structured criteria for pass/fail)
...
| --- | --- | --- |
| SO1 — Spec Ingestion | FR-SPEC-01 through 04, NF-SPEC-01 | Partially validated (AoC YAML parser works; ADR multi-format parsing not started) |
